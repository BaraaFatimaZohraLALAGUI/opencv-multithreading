...
#     SPACE_COL = np.ones((height, 20, 3), dtype=np.uint8) * 255
#     SPACE_COL2 = np.ones((height - 80, 20, 3), dtype=np.uint8) * 255
#     SPACE_ROW = np.ones((20, width * NUM_CHANNELS + 20 * 4, 3), dtype=np.uint8) * 255
#     SPACE = np.ones((height, width, 3), dtype=np.uint8) * 255
#     SPACE2 = np.ones((height - 80, width, 3), dtype=np.uint8) * 255
...
#             frames = [np.hstack((SPACE_COL, frames_dict[i])) for i in range(1, NUM_CHANNELS + 1)]
#             bgs = [np.hstack((SPACE_COL2, cv.cvtColor(fgMasks_buffer[i], cv.COLOR_GRAY2BGR))) for i in range(1, NUM_CHANNELS + 1)]

#             display_fr = np.hstack(frames)
#             display_bg = np.hstack(bgs)

#             display = np.vstack((display_fr, SPACE_ROW, display_bg))  # Stack original and mask frames vertically
