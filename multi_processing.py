    ...
    while True:
        if len(fgMasks_buffer) > 0:
            ...
            if ch_counts[current_max_idx] > COUNT_THRESHOLD:
                ...
                bound_fr_fg[0] = cv.rectangle(frames_dict[prv_max_idx + 1], (x, y), (x + w, y + h), (0, 255, 0), 2)
    ...
    while True:
        if all(i in frames_dict and i in fgMasks_buffer for i in range(1, NUM_CHANNELS + 1)):
            try:
                ...
                display_frame[height * 2 + SPACE_ROW * 2 - CLOCK_HEIGHT:height * 3 + SPACE_ROW * 2 - CLOCK_HEIGHT * 2, width + SPACE_COL:width * 2 + SPACE_COL, :] = bound_fr_fg[0][80:, :, :] if bound_fr_fg[0] is not None else SPACE2
                # display_frame[height * 2 + SPACE_ROW * 2 - CLOCK_HEIGHT:height * 3 + SPACE_ROW * 2 - CLOCK_HEIGHT * 2, width * 2 + SPACE_COL * 2:width * 3 + SPACE_COL * 2, :] = cv.cvtColor(bound_fr_fg[1], cv.COLOR_GRAY2BGR) if bound_fr_fg[1] is not None else SPACE2
                display_frame[height * 2 + SPACE_ROW * 2 - CLOCK_HEIGHT:height * 3 + SPACE_ROW * 2 - CLOCK_HEIGHT * 2, width * 2 + SPACE_COL * 2:width * 3 + SPACE_COL * 2, :] = bound_fr_fg[1] if bound_fr_fg[1] is not None else SPACE2

            except KeyError as e:
                print(f"Frame data is missing for channel: {e}")
                continue

            # Show the multi-channel video feed
